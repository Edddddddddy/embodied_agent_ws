...
- ROS 2 工程化：candidate、guarded command、feedback/result 全链路自定义 msg/action；C++ ActionScheduler 统一负责 FIFO、Action Client、优先取消、取消 watchdog 和 diagnostics，并配合 ActionGuard、Lifecycle、BehaviorTree.CPP 与 pluginlib executor。
...
embodied_agent_ws/
├── src/
│   ├── embodied_agent_interfaces/   # RobotCommand/Feedback/Result msg 与 ExecuteRobotCommand.action
