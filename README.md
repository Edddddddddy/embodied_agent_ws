...
离线评测算法现已通过两个 C++ Lifecycle component 接到实时 `/scan`：
...
`LiveLidarLoopVerifier` 以时间戳维护有界历史扫描缓存，并调用粗到细 trimmed ICP、双向重叠率、
可观测性和歧义门限；`LidarLoopVerifierNode` 将结果发布到 `/slam/loop_verifications`。跨 topic
没有全序保证，因此候选先到时会短暂 pending，查询帧到达后继续验证。`mapping_baseline.launch.py`
默认并行启动两条 shadow 旁路；两类消息始终携带 `shadow_only=true`，不会改变
slam_toolbox/Ceres/GTSAM 基线。运行时契约验收：
...
| LiDAR 在线候选+几何验证 | `bash scripts/acceptance_test.sh lidar-loop-runtime` | ROS 2；合成 LaserScan，无 Gazebo |
