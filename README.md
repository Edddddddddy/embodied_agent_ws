...
- 中间件契约：Python `ros_qos.py` 与 C++ `qos_profiles.hpp` 共享 `command/event/state/sensor/audio/diagnostics` 六类 QoS 语义；语音前端不再手写 DDS 策略。音频指标、VAD/KWS、仿真状态、动作 ACK 和 BehaviorTree 状态使用自定义消息；JSON 只保留在离线报告/JSONL 证据文件中。
