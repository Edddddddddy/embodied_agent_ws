...
def test_offline_showcase_report_generates_json_and_markdown(tmp_path):
    ...
    completed = subprocess.run(
        [
            "python3",
            str(ROOT / "tools" / "evaluation" / "generate_offline_showcase_report.py"),
            "--json-output",
            str(json_output),
            "--md-output",
            str(md_output),
...
def test_offline_showcase_report_classifies_claim_evidence(tmp_path):
    ...
    model_present = (ROOT / "models" / "Qwen3-0.6B-Q8_0.gguf").is_file()
    assert by_key["q8_gguf_model"]["status"] == (
        "proven" if model_present else "missing"
    )
