...
同参数回放得到 488 节点/491 约束固定图，在线回放轨迹 ATE 为 0.2513 m。前端只接受 1 条 closure，
该边相对位姿残差满足真值门槛，但时间间隔仅 6.84 秒，因此 accepted-edge 长回访 recall 仍为 0。
扫描证据层为唯一非局部边得到 0.8546 重叠率；在 0.65/1 m 参数下四组均不删边，固定图离线
优化后的 ATE 都是 0.1484 m。这个零变化结果是“可运行但收益不充分”，不能包装成正向提升。
