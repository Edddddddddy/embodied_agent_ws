...
- 默认链路主要依赖 energy VAD、ASR commit delay、短命令补全和 monitor 兜底。
- WebRTC VAD/AEC/NS、Silero VAD、openWakeWord/sherpa KWS 仍是 seam 或可选项，不是默认强依赖。

优化：

- 引入一个成熟默认 VAD/KWS 方案，优先选择部署成本低、可离线运行的方案。
...
- service 能避免每句重新启动进程/加载模型，但 CPU 合成仍是秒级。
- 当前 `<300ms` TTS gate 仍以 Sherpa-TTS 为准。

优化：

- 做短文本缓存，例如“收到”“正在执行”“已停止”。
- 评估量化、分段合成或更快声码器。
...
- 地图、目标点和场景资产仍比较静态。
- failure reason 仍可细化，例如 planner/controller/localization 失败。

优化：

- 增加固定 Gazebo world、map、waypoint assets。
- 增加 RViz 展示脚本，突出 TF、map、path、goal。
- 对 Nav2 result 做更细粒度报告。
