...
| 连续演示 summary | 当前 | monitor 退出时汇总 wake/sleep/retry、ASR、ignored、normalized、queue、execution、result 和最近窗口 audio profile 建议，`CONTINUOUS_MONITOR_AUDIO_SAMPLE_LIMIT` 可配置音频滑动窗口 |
| 连续脚本配置预览 | 当前 | `continuous_voice_control.sh` 支持 `CONTINUOUS_PRINT_CONFIG=true` 无麦克风 dry-run，并透传 WAKE/AEC/NS/AGC/READINESS 环境变量 |
...
| 连续语音去噪 | 当前 | 会话层忽略短 filler，并对短窗口内重复 ASR final 去重；`CONTINUOUS_DUPLICATE_WINDOW_S` 可调去重窗口，`/agent/recognition_feedback` 与 monitor 可观察过滤原因 |
