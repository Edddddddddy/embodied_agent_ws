...
if str(SCRIPTS_DIR) not in sys.path:
    # 版本探针仍属于部署脚本；评估报告显式接入它，避免依赖调用者的 cwd/PYTHONPATH。
...
def collect_asr_tts_benchmark(run_asr_tts: bool, timeout_s: float) -> dict[str, Any]:
    ...
    return _run_json_command([sys.executable, "tools/evaluation/benchmark_offline.py"], timeout_s=timeout_s)
...
def collect_llama_decode_benchmark(
    ...
    command = [sys.executable, "tools/evaluation/benchmark_llama_decode_speed.py"]
...
def collect_instruction_following(
    ...
    command = (
        [sys.executable, "tools/evaluation/evaluate_instruction_following.py"]
        if input_report
        else ["bash", "tools/evaluation/evaluate_instruction_following.sh"]
    )
    if input_report:
...
def build_claim_evidence(report: dict[str, Any]) -> dict[str, Any]:
    ...
    items = [
        ...
        _claim(
            ...
            next_step="运行 python3 tools/evaluation/generate_offline_showcase_report.py --run-latency。",
        ),
        _claim(
        ...
        _claim(
            ...
            next_step="运行 python3 tools/evaluation/generate_offline_showcase_report.py --run-asr-tts。",
        ),
        _claim(
