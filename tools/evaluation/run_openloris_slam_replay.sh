...
if [[ ! -s "$BAG_SOURCE" ]]; then
  ...
  echo "Prepare the bag with tools/evaluation/setup_openloris_rosbag.py before claiming real-data evidence." >&2
  exit 2
fi
...
if [[ ! -s "$REFERENCE" ]]; then
  python3 tools/evaluation/setup_openloris_groundtruth.py --sequence "$SEQUENCE" --output-root "$ROOT"
fi
...
python3 tools/evaluation/evaluate_slam_trajectory.py "${EVAL_ARGS[@]}"
DEGRADATION_ARGS=(
...
python3 tools/evaluation/analyze_slam_degradation.py "${DEGRADATION_ARGS[@]}"
if [[ "${OPENLORIS_EVALUATE_FRONTEND:-false}" == "true" ]]; then
  if [[ ! -s "$FRONTEND_LOG" ]]; then
    echo "FAIL: missing loop-frontend trace: $FRONTEND_LOG" >&2
    exit 1
  fi
  python3 tools/evaluation/analyze_loop_frontend_trace.py \
...
if [[ "$BACKEND" == "gtsam" && "${OPENLORIS_EVALUATE_LOOP_CONSTRAINTS:-false}" == "true" ]]; then
  ...
  python3 tools/evaluation/evaluate_loop_constraints.py "${LOOP_ARGS[@]}"
fi
LOOP_MANIFEST_ARGS=()
...
python3 tools/evaluation/build_openloris_experiment_manifest.py \
