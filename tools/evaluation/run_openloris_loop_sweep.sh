...
if [[ -e "$DERIVED_BAG" || -e "$DERIVED_SOURCE" ]]; then
  ...
  python3 tools/evaluation/compact_openloris_rosbag.py \
...
mkdir -p "$RESULT_ROOT"
python3 tools/evaluation/build_openloris_loop_sweep_configs.py \
...
python3 tools/evaluation/compare_openloris_loop_sweep.py \
