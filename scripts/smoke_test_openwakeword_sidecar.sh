#!/usr/bin/env bash
set -euo pipefail
WORKSPACE="${WORKSPACE:-/home/ubuntu/embodied_agent_ws}"
source "$WORKSPACE/scripts/activate.sh"
export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-$((190 + $$ % 30))}"

FAKE_PACKAGE_DIR="$(mktemp -d)"
LOG_FILE="$(mktemp)"

python3 - "$FAKE_PACKAGE_DIR" <<'PY'
from pathlib import Path
import sys

root = Path(sys.argv[1])
package = root / "openwakeword"
package.mkdir(parents=True)
(package / "__init__.py").write_text("", encoding="utf-8")
(package / "model.py").write_text(
    """
class Model:
    def __init__(self, **kwargs):
        self.kwargs = kwargs

    def predict(self, audio):
        # fake_wake 始终高于阈值，用于验证 ROS sidecar runtime 链路。
        return {"fake_wake": 0.91, "noise": 0.01}
""".lstrip(),
    encoding="utf-8",
)
PY

PYTHONPATH="$FAKE_PACKAGE_DIR:${PYTHONPATH:-}" \
setsid ros2 run embodied_voice_frontend keyword_wake --ros-args \
  -p mode:=openwakeword \
  -p provider_name:=openwakeword_test \
  -p openwakeword_threshold:=0.5 \
  -p openwakeword_inference_framework:=onnx \
  >"$LOG_FILE" 2>&1 &
KWS_PID=$!

cleanup() {
  kill -TERM -- "-$KWS_PID" 2>/dev/null || true
  sleep 0.2
  kill -KILL -- "-$KWS_PID" 2>/dev/null || true
  wait "$KWS_PID" 2>/dev/null || true
  rm -rf "$FAKE_PACKAGE_DIR"
  rm -f "$LOG_FILE"
}
trap cleanup EXIT

if ! timeout 15 bash "$WORKSPACE/tests/integration/run_probe.sh" "$WORKSPACE/tests/integration/voice/test_openwakeword_sidecar.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi

echo "PASS: openWakeWord keyword wake sidecar consumes audio and publishes wake_event_input"
