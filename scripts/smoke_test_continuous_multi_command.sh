...
if ! timeout 45 bash "$WORKSPACE/tests/integration/run_probe.sh" "$WORKSPACE/tests/integration/voice/test_continuous_multi_command.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi
echo "PASS: NLU multi-command ASR final -> queued ordered actions ($AGENT_KIND)"
