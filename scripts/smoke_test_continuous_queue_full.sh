...
export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-$((220 + $$ % 10))}"
...
if ! timeout 35 bash "$WORKSPACE/tools/acceptance/run_probe.sh" "$WORKSPACE/tools/acceptance/probes/voice/continuous_queue_full.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi
echo "PASS: bounded continuous queue publishes queue_full rejection and feedback ($AGENT_KIND)"
