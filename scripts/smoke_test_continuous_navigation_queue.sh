...
export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-$((175 + $$ % 20))}"
...
if ! timeout 55 bash "$WORKSPACE/tests/integration/run_probe.sh" "$WORKSPACE/tests/integration/slam_nav/test_continuous_navigation_queue.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi
