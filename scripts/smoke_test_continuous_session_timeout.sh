...
export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-$((230 + $$ % 3))}"
...
if ! timeout 35 bash "$WORKSPACE/tools/acceptance/run_probe.sh" "$WORKSPACE/tools/acceptance/probes/voice/continuous_session_timeout.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi
echo "PASS: continuous session timeout rejects stale commands and accepts a fresh wake ($AGENT_KIND)"
