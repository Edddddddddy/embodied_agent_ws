...
setsid ros2 launch embodied_simulation voice_turtlebot3.launch.py \
  ...
  use_typed_actions:=true >"$LAUNCH_LOG" 2>&1 &
LAUNCH_PID=$!

# JSON/legacy 动作链已删除；probe 自身强制验证 typed ROS 2 Action result。
if ! bash "$WORKSPACE/tools/acceptance/run_probe.sh" "$WORKSPACE/tools/acceptance/probes/voice/gazebo_voice.py"; then
  cat "$LAUNCH_LOG" >&2
  cat "$SERVER_LOG" >&2
  exit 1
fi
echo "PASS: speech -> ZipFormer -> llama.cpp -> typed ActionGuard -> Gazebo TurtleBot3"
