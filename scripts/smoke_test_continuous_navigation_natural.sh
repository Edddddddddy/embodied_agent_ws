...
export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-$((195 + $$ % 20))}"
...
if [[ "$AGENT_KIND" == "online" ]]; then
  setsid ros2 run embodied_online_agent online_agent --ros-args \
    ...
    -p action_sequence_wait_timeout_s:=18.0 \
    >>"$LOG_FILE" 2>&1 &
else
...
if ! timeout 55 bash "$WORKSPACE/tools/acceptance/run_probe.sh" "$WORKSPACE/tools/acceptance/probes/slam_nav/continuous_navigation_natural.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi
echo "PASS: natural speech -> waypoint patrol queue ($AGENT_KIND)"
exit 0
