...
for sequence in "${sequence_list[@]}"; do
  ...
  python3 scripts/extract_openloris_scan_corpus.py \
    --graph "$graph" \
    --bag "$bag" \
  ...
  python3 scripts/evaluate_lidar_loop_candidates.py \
...
python3 scripts/compare_lidar_loop_candidate_sequences.py \
  "${aggregate_args[@]}" \
