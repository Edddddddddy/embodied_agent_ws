...
if [[ "$AGENT_KIND" == "online" ]]; then
  setsid ros2 run embodied_online_agent online_agent --ros-args \
    ...
    -p continuous_command_queue_size:=16 \
    -p voice_session_timeout_s:=90.0 \
...
if ! timeout 70 bash "$WORKSPACE/tests/integration/run_probe.sh" "$WORKSPACE/tests/integration/voice/test_continuous_voice_soak.py"; then
  cat "$LOG_FILE" >&2
  exit 1
fi
echo "PASS: long continuous session keeps accepting queued commands ($AGENT_KIND)"
